...
In addition to the Python CLI, the pipeline can be driven through **Claude Code skills** — self-contained verification agents that communicate with deterministic Python tools via an MCP (Model Context Protocol) server.
...
│    │                                                      │
│    ├─→ get_paper_from_dataset  (MCP)                      │
│    ├─→ segment_paper           (MCP)                      │
│    │                                                      │
│    │  For each snippet, invokes:                          │
│    ├─→ /verify-math       → run_sympy_sandbox (MCP)       │
│    ├─→ /verify-text                                         │
│    ├─→ /verify-vision                                       │
│    ├─→ /verify-statistical → safe_eval (MCP)                │
│    ├─→ /verify-citation                                     │
│    └─→ /verify-triage     (uncertainty mode)                │
│                                                           │
│    Aggregates findings → final verdict                    │
└──────────────┬───────────────────────────────────────────┘
...
│                                                           │
│  Tools exposed:                                            │
│    parse_paper          — Raw content → NormalizedPaper    │
│    segment_paper        — NormalizedPaper → Snippets       │
│    run_sympy_sandbox    — Execute SymPy code safely        │
│    safe_eval            — Injection-safe arithmetic        │
│    get_paper_from_dataset — Fetch paper by ID              │
│    analyze_schema       — Inspect dataset schema           │
│    get_error_annotations — Fetch ground-truth annotations  │
│    fuzzy_match_locations — Compare location strings        │
│    chunk_text           — Boundary-aware text chunking     │
└──────────────────────────────────────────────────────────┘
```

### Design Rationale

The skill + MCP split follows a deliberate separation of concerns:

- **Skills (`.claude/skills/`)** — Handle reasoning, judgment, and coordination. Each skill defines a system prompt and workflow that tells Claude *how* to verify a specific type of content. Skills use Claude's built-in LLM for the heavy cognitive work.

- **MCP Server (`mcp-server/server.py`)** — Handles deterministic, mechanical operations that are better done in code: parsing, segmentation, safe math execution, data access. These are exposed as tools that skills invoke via JSON-RPC.

This separation:
1. **Keeps skills focused** — A skill's SKILL.md is pure prompt engineering, not code
2. **Prevents hallucination** — Math is executed in a real SymPy sandbox, not generated by the LLM
3. **Enables reuse** — The same MCP tools serve the Python CLI, the skills, and any future frontend
4. **Makes skills testable** — Skills can be tested with the mock MCP server

### Skill Composition

The `/verify-paper` skill orchestrates the other six skills:

```
/verify-paper
  │
  ├─ 1. get_paper_from_dataset  → fetch raw paper
  ├─ 2. segment_paper           → produce snippets
  │
  ├─ 3. [exhaustive mode] Route EVERY snippet by type:
  │     EQUATION    → /verify-math
  │     FIGURE/TABLE → /verify-vision
  │     SECTION/... → /verify-text
  │
  ├─ 3. [uncertainty mode] Triage first:
  │     /verify-triage on all snippets → uncertainty map
  │     Then route only high-uncertainty snippets to specialists
  │
  └─ 4. Aggregate → final report with verdict
```

Each specialist skill returns structured findings. The orchestrator aggregates them, filters by confidence, and compares against ground truth when available.

### Batch Verification

Two scripts automate running `/verify-paper` across many papers:

- **`scripts/batch-verify.sh`** — Shell script; calls `claude -p` for each paper
- **`scripts/batch_verify.py`** — Python script; adds progress bars, JSON parsing, and optional `--evaluate` pass

Both support `--papers`, `--offset`, `--mode`, `--threshold`, `--dry-run`, `--paper-id`, and `--list`.
...
| MCP Server | Tool errors → JSON error response (never crashes the server) |
| Claude Code Skill | Skill failures → structured error in verification report |
